...
$query = "SELECT * from updateaktivitasmahasiswa where err_no is null or err_no != '0' order by id ASC";
// echo $query;
...
if (mysqli_num_rows($hasil) > 0) {
    while ($x = mysqli_fetch_array($hasil)) {
        ...
        print_r($data);
        file_put_contents('1. TXT\log_payload_UpdateAktivitasMahasiswa.txt', print_r($data, true), FILE_APPEND); // Logging payload
        file_put_contents('1. JSON\log_payload_UpdateAktivitasMahasiswa.json', print_r($data, true), FILE_APPEND); // Logging payload
        ob_flush();
        flush();
        ...
        print_r($ws_result);
        file_put_contents('1. TXT\log_response_UpdateAktivitasMahasiswa.txt', print_r($ws_result, true), FILE_APPEND); // Logging respons
        file_put_contents('1. JSON\log_response_UpdateAktivitasMahasiswa.json', print_r($ws_result, true), FILE_APPEND); // Logging respons
        ob_flush();
        flush();
        ...
        if ($err_code == 0) {
            ...
            $update = "UPDATE updateaktivitasmahasiswa SET err_no='$err_code', err_desc='$err_desc' WHERE id=$id";
