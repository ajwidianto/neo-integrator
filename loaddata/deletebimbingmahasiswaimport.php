...
$query = "select * from progress where act='DeleteBimbingMahasiswa'";
...
$no = 1;
if ($show == 'yes') {
    $tampil = '<a href="?module=inject&act=deletebimbingmahasiswaimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from deletebimbingmahasiswa";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>ID Bimbing Mahasiswa</th>
        ...
        while ($x = mysqli_fetch_array($hasil)) {
            ...
            echo "<tr><td>" . $no++;
            echo "</td><td>" . $x['id_bimbing_mahasiswa'];
            echo "</td><td>" . $x['err_no'];
    ...
    $tampil = '<a href="?module=inject&act=deletebimbingmahasiswaimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from deletebimbingmahasiswa where err_no='0'";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>ID Bimbing Mahasiswa</th>
    ...
    SUM(err_no IS NOT NULL AND err_no!='0') AS gagal FROM deletebimbingmahasiswa;";
    ...
    $tampil = '<a href="?module=inject&act=deletebimbingmahasiswaimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from deletebimbingmahasiswa where err_no!='0' and err_no is not null";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>ID Bimbing Mahasiswa</th>
    ...
    $tampil = '<a href="?module=inject&act=deletebimbingmahasiswaimport"><button type="button" class="btn btn-primary">
...
echo '<div class="card-body">
    <div class="buttons">
        ...
        <a href="?module=inject&act=deletebimbingmahasiswaimport&show=berhasil">
        ...
        <a href="?module=inject&act=deletebimbingmahasiswaimport&show=gagal">
