...
$query = "select * from progress where act='InsertBimbingMahasiswa'";
...
$no = 1;
if ($show == 'yes') {
    $tampil = '<a href="?module=inject&act=bimbingaktivitasmhsimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from insertbimbingaktivitasmahasiswa";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>ID Aktivitas</th><th>NIDN/NUPTK</th><th>NAMA DOSEN</th><th>ID DOSEN</th><th>Kategori Kegiatan</th><th>Pembimbing Ke-</th><th>Err No</th><th>Err Desc</th></tr>";
    ...
    $tampil = '<a href="?module=inject&act=bimbingaktivitasmhsimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from insertbimbingaktivitasmahasiswa where err_no='0'";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
    ...
    SUM(err_no IS NOT NULL AND err_no!='0') AS gagal FROM insertbimbingaktivitasmahasiswa;";
    ...
    $tampil = '<a href="?module=inject&act=bimbingaktivitasmhsimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from insertbimbingaktivitasmahasiswa where err_no!='0' and err_no is not null";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>ID Aktivitas</th><th>NIDN/NUPTK</th><th>NAMA DOSEN</th><th>ID DOSEN</th><th>Kategori Kegiatan</th><th>Pembimbing Ke-</th></tr>";
    ...
    $tampil = '<a href="?module=inject&act=bimbingaktivitasmhsimport"><button type="button" class="btn btn-primary">
...
echo '<div class="card-body">
    <div class="buttons">
        ...
        <a href="?module=inject&act=bimbingaktivitasmhsimport&show=berhasil">
        ...
        <a href="?module=inject&act=bimbingaktivitasmhsimport&show=gagal">
