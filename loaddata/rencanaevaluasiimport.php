...
$query = "select * from progress where act='InsertRencanaEvaluasi'";
...
$no = 1;
if ($show == 'yes') {
    $tampil = '<a href="?module=inject&act=rencanaevaluasiimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from insertrencanaevaluasi";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>ID Kelas Kuliah</th><th>ID Jenis Evaluasi</th><th>Nama Evaluasi</th><th>Deskripsi Indonesia</th><th>Deskripsi Inggris</th><th>Nomor Urut</th><th>Bobot Evaluasi (%)</th><th>Err No</th><th>Err Desc</th></tr>";
        while ($x = mysqli_fetch_array($hasil)) {
            ...
            echo "</td><td>" . $x['nama_evaluasi'];
            echo "</td><td>" . $x['deskripsi_indonesia'];
            echo "</td><td>" . $x['deskripsi_inggris'];
    ...
    $tampil = '<a href="?module=inject&act=rencanaevaluasiimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from insertrencanaevaluasi where err_no='0'";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>ID Kelas Kuliah</th><th>ID Jenis Evaluasi</th><th>Nama Evaluasi</th><th>Deskripsi Indonesia</th><th>Deskripsi Inggris</th><th>Nomor Urut</th><th>Bobot Evaluasi (%)</th><th>Err No</th><th>Err Desc</th></tr>";
    ...
    SUM(err_no IS NOT NULL AND err_no!='0') AS gagal FROM insertrencanaevaluasi;";
    ...
    $tampil = '<a href="?module=inject&act=rencanaevaluasiimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from insertrencanaevaluasi where err_no!='0' and err_no is not null";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>ID Kelas Kuliah</th><th>ID Jenis Evaluasi</th><th>Nama Evaluasi</th><th>Deskripsi Indonesia</th><th>Deskripsi Inggris</th><th>Nomor Urut</th><th>Bobot Evaluasi (%)</th><th>Err No</th><th>Err Desc</th></tr>";
    ...
    $tampil = '<a href="?module=inject&act=rencanaevaluasiimport"><button type="button" class="btn btn-primary">
...
echo '<div class="card-body">
    <div class="buttons">
        ...
        <a href="?module=inject&act=rencanaevaluasiimport&show=berhasil">
        ...
        <a href="?module=inject&act=rencanaevaluasiimport&show=gagal">
