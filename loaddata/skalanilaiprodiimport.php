...
$query = "select * from progress where act='InsertSkalaNilaiProdi'";
...
$no = 1;
if ($show == 'yes') {
    $tampil = '<a href="?module=inject&act=skalanilaiprodiimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from insertskalanilaiprodi";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>Jenjang Pendidikan</th><th>Nama Prodi</th><th>Nilai Huruf</th><th>Nilai Indekx</th><th>Bobot Minimum</th><th>Bobot Maksimum</th><th>Tanggal Mulai Efektif</th><th>Tanggal Akhir Efektif</th><th>Error</th><th>Description</th></tr>";
        while ($x = mysqli_fetch_array($hasil)) {
            ...
            // echo "</td><td>" . $x['kode_program_studi'];
            echo "</td><td>" . $x['nama_jenjang_pendidikan'];
            echo "</td><td>" . $x['nama_program_studi'];
            echo "</td><td>" . $x['nilai_huruf'];
            echo "</td><td>" . $x['nilai_indeks'];
            echo "</td><td>" . $x['bobot_minimum'];
            echo "</td><td>" . $x['bobot_maksimum'];
            echo "</td><td>" . $x['tanggal_mulai_efektif'];
            echo "</td><td>" . $x['tanggal_akhir_efektif'];
            echo "</td><td>" . $x['err_no'];
    ...
    $tampil = '<a href="?module=inject&act=skalanilaiprodiimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from insertskalanilaiprodi where err_no='0'";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>Jenjang Pendidikan</th><th>Nama Prodi</th><th>Nilai Huruf</th><th>Nilai Indekx</th><th>Bobot Minimum</th><th>Bobot Maksimum</th><th>Tanggal Mulai Efektif</th><th>Tanggal Akhir Efektif</th><th>Error</th><th>Description</th></tr>";
    ...
    SUM(err_no IS NOT NULL AND err_no!='0') AS gagal FROM insertskalanilaiprodi;";
    ...
    $tampil = '<a href="?module=inject&act=skalanilaiprodiimport&show=no"><button type="button" class="btn btn-danger">
    SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
    $query = "select * from insertskalanilaiprodi where err_no!='0' and err_no is not null";
    $hasil = mysqli_query($db, $query);
    if (mysqli_num_rows($hasil) > 0) {
        echo "<table class='table table-striped' border='1'><tr>
        <th>Baris</th><th>Jenjang Pendidikan</th><th>Nama Prodi</th><th>Nilai Huruf</th><th>Nilai Indekx</th><th>Bobot Minimum</th><th>Bobot Maksimum</th><th>Tanggal Mulai Efektif</th><th>Tanggal Akhir Efektif</th><th>Error</th><th>Description</th></tr>";
    ...
    $tampil = '<a href="?module=inject&act=skalanilaiprodiimport"><button type="button" class="btn btn-primary">
...
echo '<div class="card-body">
    <div class="buttons">
        ...
        <a href="?module=inject&act=skalanilaiprodiimport&show=berhasil">
        ...
        <a href="?module=inject&act=skalanilaiprodiimport&show=gagal">
